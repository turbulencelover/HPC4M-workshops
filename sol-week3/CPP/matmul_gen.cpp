...
// If N is not divisible by 'nproc' (this code cannot handle it) -> abort!
  if((N % nproc) != 0) {
    cout<<"N = "<<nproc<<" is not divisible by number of processes = "<< nproc;
